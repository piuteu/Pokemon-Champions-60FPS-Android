...
- APKは改変せず、Google Play版をそのまま使用します。
- 通常利用にFridaは必要ありません。
- 現在の検証対象は、Pokémon Champions Android 1.1.5 /
  `versionCode 3191`です。
- 検証環境は、BlueStacks Android 13 / Tiramisu64とReZygisk互換root環境です。

その他の端末や構成への互換性は主張していません。

## 概要

このプロジェクトは、正規のGoogle Play版をそのまま使い、ゲーム起動後の
ランタイムで、フレームレート制御と、検証済みのアニメーションフレームレート
経路を調整します。APKの再パッケージ化、patch、re-sign、ゲームの再配布は
行いません。ユーザー向けの推奨配布物は、メンテナーがビルドしたReZygisk互換
モジュールZIPです。ソースからのビルド手順は、研究、検証、コントリビューター
向けに用意されています。

対象はフレームレート関連のランタイム入力と、検証済みのアニメーションフレーム
レート経路です。ゲームプレイの内容、ネットワーク通信、セーブデータ、
ポケモンデータを変更するものではありません。
