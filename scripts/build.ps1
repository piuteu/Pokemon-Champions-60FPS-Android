...
$compilerRelativePath = 'toolchains\llvm\prebuilt\windows-x86_64\bin\aarch64-linux-android33-clang++.cmd'
...
$source = Join-Path $repoRoot 'src\pc_fps_runtime.cpp'
$outputDirectory = Join-Path $repoRoot 'build'
$output = Join-Path $outputDirectory 'libpcfps_runtime.so'

if (-not (Test-Path -LiteralPath $compiler -PathType Leaf)) {
    throw "ARM64 clang++ compiler not found: $compiler"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "ARM64 payload build failed with exit code $LASTEXITCODE"
}
